...
async function migratePhotos() {
  try {
    ...
    for (const wedding of realWeddings) {
      ...
      if (contentfulEntry.fields.coverImage || contentfulEntry.fields.heroImage) {
      ...
      // Upload LIMITED gallery photos (just first 10 to keep it manageable)
      if (wedding.galleries && wedding.galleries.length > 0) {
        const allPhotos = wedding.galleries.flatMap(gallery => gallery.photos)
        const photosToUpload = allPhotos.slice(0, 10) // Just 10 for now
    ...
    console.log('\n📊 Summary:')
    console.log('   - Weddings processed:', realWeddings.length)
    console.log('   - Each wedding got: hero, cover, featured + 10 gallery photos')
    console.log('\n✅ All photos have been uploaded to Contentful!')
...
console.log('  CONTENTFUL PHOTO MIGRATION (AUTO-RUN)')
console.log('='.repeat(50))
console.log()
console.log('Uploading wedding photos to Contentful...')
console.log('This will upload hero, cover, featured + 10 gallery photos per wedding.')
console.log()
