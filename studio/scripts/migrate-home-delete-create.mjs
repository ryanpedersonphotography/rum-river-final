...
const client = createClient({
  ...
  token: process.env.SANITY_AUTH_TOKEN, // provided by --with-user-token or env
...
async function run() {
  ...
  // If the old document has the same ID but wrong type, we need to delete it first
  if (oldHome._id === 'homepage' && oldHome._type !== 'homePage') {
    console.log('→ Deleting old homepage document with wrong type...')
    await client.delete('homepage')
